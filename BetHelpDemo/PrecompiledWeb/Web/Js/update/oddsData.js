...
var LoadOddsUpdateRQ = function (node) {
    var companyid_array = [];
    ...
    var fields = [{ name: 'id', type: 'int' },
            ...
            { name: 'pankou', type: 'float' },
            { name: 'away', type: 'float'}];

    var store = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy(
           {
               url: "Data/NowGoal/odds.aspx?a=rqlist",
               method: "POST"
           }),
                    ...
                    var win = new Ext.Window({
                        ...
                        buttons: [{
                            ...
                            handler: function () {
                                ...
                                var updateOdds = function (scheduleid, companyid) {
                                    Ext.fly('pbar1_text').update('正在更新 ' + scheduleid + '  已更新' + scheduleIndex + "/" + scheduleid_arr.length + '条...');
                                    Ext.fly('pbar2_text').update('正在更新 ' + company[companyid] + ' 的数据');
                                    Ext.Ajax.request({
                                        url: 'Server.aspx?a=updateOdds',
                                        params: { scheduleid: scheduleid, companyid: companyid },
                                        timeout:600*1000,
                                        success: function (res) {
                                            var result = Ext.decode(res.responseText);
                                            if (result.success) {
                                                ...
                                                pbar1.updateProgress(per, Math.round(100 * per) + '% 完成...');
                                                if (scheduleIndex >= scheduleid_arr.length) {
                                                    Ext.fly('pbar1_text').update('更新完成');
                                                    Ext.fly('pbar2_text').update('更新完成');
                                                    store.reload();
                                                } else {
                                                    updateOdds(scheduleid_arr[scheduleIndex], companyid_array[companyIndex]);
                                                }
                                            } else {
                                                Ext.fly('pbar1_text').update('出现错误：' + result.message);
                                                Ext.fly('update-btn').dom.disabled = false;
    ...
    var tab = center.getItem("ScheduleOddsDataTab");
    if (!tab) {
        var tab = center.add({
            id: "ScheduleOddsDataTab",
            iconCls: "totalicon",
            xtype: "panel",
            title: node.text,
            closable: true,
